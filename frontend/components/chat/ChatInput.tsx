...
interface ChatInputProps {
  onSend: (content: string, type: 'NORMAL' | 'SCHEDULE_REQUEST') => void;
  isPending?: boolean;
...
export function ChatInput({ onSend, isPending = false, maxContentLength = 2000 }: ChatInputProps) {
  ...
  const [messageType, setMessageType] = useState<'NORMAL' | 'SCHEDULE_REQUEST'>('NORMAL');
  ...
  const toggleMessageType = () => {
    setMessageType((prev) =>
      prev === 'NORMAL' ? 'SCHEDULE_REQUEST' : 'NORMAL'
    );
  };
